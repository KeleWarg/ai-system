...
import { hexToHSL } from '@/lib/color-converter'
              ...
              <div>
                ...
                <div className="mt-2 space-y-2">
                  {extractedData.colors.map((colorEntry: string, i: number) => {
                    // Parse "Purpose: #HEXCODE" or just "#HEXCODE"
                    const match = colorEntry.match(/^(.+?):\s*(#[A-Fa-f0-9]{6}|#[A-Fa-f0-9]{3})$/)
                    const purpose = match ? match[1].trim() : 'Color'
                    const hexColor = match ? match[2] : colorEntry
                    
                    let hslColor = ''
                    try {
                      hslColor = hexToHSL(hexColor)
                    } catch (e) {
                      hslColor = 'Invalid hex'
                    }
